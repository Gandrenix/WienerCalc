// Informa a TypeScript que 'anime' y 'autoAnimate' existirán globalmente
declare const anime: (params: any) => any;
declare const autoAnimate: (element: HTMLElement, config?: any) => any;

/// <reference path="./preload.d.ts" />
/// <reference types="echarts" />

// --- NUEVO: Importaciones para Animación ---
import * as echarts from 'echarts'; // *** CORRECCIÓN *** (Importación añadida)
...
 */
function shakeElement(target: string | HTMLElement | null) {
    if (!target) return; // *** CORRECCIÓN *** Añadido guard para null
    anime({ // *** CORRECCIÓN *** (Llamada casteada a 'any')
        targets: target,
        translateX: [
...
function setupTabs() {
    ...
    tabLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            const targetId = (link as HTMLAnchorElement).hash; // ej. "#library"
            if (!targetId) return;
            ...
            if (targetPanel) {
                (targetPanel as HTMLElement).hidden = false;
            }
...
    	if (importLogCsvBtn) importLogCsvBtn.disabled = false; // *** NUEVO: Habilitar ambos ***
  	}
}
